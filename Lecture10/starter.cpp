...
Fraction operator+(const Fraction& lhs, const Fraction& rhs) {
     ...
     */
    double result_numerator = lhs.numerator * rhs.denominator + lhs.denominator * rhs.numerator;
    double result_denominator = lhs.denominator * rhs.denominator;
...
std::ostream& operator<<(std::ostream& os, const Fraction& f) {
    ...
    else if((f.numerator < 0 && f.denominator > 0) || (f.numerator > 0 && f.denominator < 0)){
        os << "-" << f.numerator << " / " << f.denominator;
    }
    return os;
}
