...
public:
    ...
    virtual float shootingSuccessRate(int base_skill) {
        return 0.5f * base_skill * (1.0f - fatigue_level);
    }
    ...
    void checkEnergy(int base_stamina) {/* Complete the rest */
        int effective_stamina = static_cast<int>(base_stamina * (1 - fatigue_level));
        std::cout << "Energy check -- Stamina: " << effective_stamina << " (Fatigue: " << std::fixed << std::setprecision(1) << (fatigue_level * 100.0f) << "%)" << std::endl;
    }
    
    void checkEnergy(float base_hydration){/* Complete the rest */
...
public:
    ...
    float shootingSuccessRate(int base_skill) override { /* Complete the rest */
    ...
    float tackleSuccessRate(int base_skill) override { /* Complete the rest */
...
int main() {
     ...
     */
    Defender* minjae = new Defender("Minjae Kim");
    minjae->checkEnergy(base_stamina_value);
    minjae->checkEnergy(base_hydration_value);
    minjae->tackleSuccessRate(base_skill_value);
